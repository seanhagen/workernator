...
Other job-runner services are built to allow *any* kind of job to get run they have to rely on things like `interface{}/any` or JSON strings to provide data to a job when it starts. This is not great; the conversion and manual type checking add effort better spent elsewhere, and additionally adds a potential bug-prone section to the code.
...
1.  CGroups & Namespaces - Resource Control and Isolation
